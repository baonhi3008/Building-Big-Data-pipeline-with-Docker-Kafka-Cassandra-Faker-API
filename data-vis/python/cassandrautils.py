...
def saveFakerDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + fakertable + "(name, address, country, credit_card_number, email, i_bank, job, " \
                                                "phone, " \
                                                "swift, year) \
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    ...
    batches = []
    for idx, val in dfrecords.iterrows():
        batch.add(insert, (val['name'], val['address'], val['country'], val['credit_card_number'], val['email'],
                           val['i_bank'], val['job'], val['phone'], val['swift'], val['year']))
        counter += 1
        if counter >= 100:
            print('inserting ' + str(counter) + ' records')
            totalcount += counter
...
def saveMimesisDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + mimesistable + "(chef, chef_nationality, dish, drink, fruit, level, quote, spices, " \
                                                "vegetable) \
                                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) "
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    insert = session.prepare(cqlsentence)
    batches = []
    for idx, val in dfrecords.iterrows():
        batch.add(insert, (val['chef'], val['chef_nationality'], val['dish'],
                           val['drink'], val['fruit'], val['level'], val['quote'], val['spices'], val['vegetable']))
        counter += 1
        if counter >= 100:
            print('inserting ' + str(counter) + ' records')
            totalcount += counter
...
def loadDF(targetfile, target):
    if target == 'weather':
        ...
        colsnames = ['chef', 'chef_nationality', 'dish', 'drink', 'fruit', 'level', 'quote', 'spices', 'vegetable']
